...
class SmartGridBotV4:
    ...
    def close_order(self, side, qty):
        ...
        try:
            ...
            if e.code == -2022:
                # Pozisyon yok — sadece muhasebeden sil, hata loglama
                log.warning(f"Pozisyon zaten kapalı (code=-2022), muhasebeden siliniyor")
                return None
    ...
    def _open(self, key, lvl, price, portfolio_val):
        ...
        # Gerçek borsa emri — başarısız olursa positions'a ekleme
        order = self.place_order(side, qty)
        if order is None:
            log.warning(f"Emir başarısız, pozisyon açılmadı: {side} ${trig:.2f}")
            return
    ...
    def run(self):
        ...
        # Başlangıçta mevcut açık pozisyonları sync et
        try:
            ...
            for pos in real_positions:
                ...
                if abs(amt) > 0.001:
                    ...
                    log.info(f"♻️  Mevcut pozisyon sync: {side} {abs(amt):.4f} @ ${ep:.2f}")
                    # Phantom key ile ekle
                    key = f"sync_{side}_{int(ep)}"
                    self.positions[key] = {
                        "side": side,
                        "qty": abs(amt),
                        "entry": ep,
                        "tp": ep * (1.004 if side=="long" else 0.996),
                    }
        except Exception as e:
            log.warning(f"Pozisyon sync hatası: {e}")
